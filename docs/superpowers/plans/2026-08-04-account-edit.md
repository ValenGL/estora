# Account Edit Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the legacy `/account` page with a role-aware inline-edit page where sellers edit their business listing and buyers edit their Buy Box criteria.

**Architecture:** Single `page.tsx` client component with two local section functions (`SellerSection`, `BuyerSection`). Role is read from `useAuth()`. Data is loaded via existing `getOwnSeller` / `getOwnBuyer` on mount. Inline edit toggle pattern — "Editar" button reveals a pre-filled form; "Guardar" calls `updateSeller` / `updateBuyer`.

**Tech Stack:** Next.js 15 App Router, TypeScript, SCSS, Supabase JS client, existing data layer in `src/app/lib/supabase/`.

## Global Constraints

- No test framework is installed — verification is `npx tsc --noEmit` + manual dev server check.
- Do NOT reference the `users` table — it is from the legacy schema. Use `profiles`, `sellers`, `buyers`.
- Do NOT modify `src/app/lib/supabase/sellers.ts` or `buyers.ts` — the data layer is complete.
- Do NOT modify `src/app/lib/types.ts` — all types are already defined.
- Do NOT create shared components — `SellerSection` and `BuyerSection` are local functions in `page.tsx`.
- All labels in Spanish (es-AR informal). Code identifiers in English.
- Follow the SCSS class naming pattern of `account-*` (not Tailwind) for new classes.
- `asking_price` and seller `status` are NOT editable in this sprint.
...
**Files:**
- Create: `src/app/account/account.scss`

**Interfaces:**
- Produces: all CSS classes consumed by `page.tsx` in Task 2

- [ ] **Step 1: Create the SCSS file**

```scss
// src/app/account/account.scss
...
```bash
cd /Users/valentingonzalez/Proyects/brokerage && npx tsc --noEmit
```

Expected: no errors (SCSS file has no TS impact).

- [ ] **Step 3: Commit**

```bash
git add src/app/account/account.scss
git commit -m "feat: add account page styles"
```

---

### Task 2: page.tsx — full rewrite

**Files:**
- Modify: `src/app/account/page.tsx` (full replacement)

**Interfaces:**
- Consumes:
  - `useAuth()` from `src/app/utils/isAuth.tsx` → `{ role: Role | null }`
  - `getOwnSeller()` → `Promise<Seller | null>` from `src/app/lib/supabase/sellers.ts`
  - `updateSeller(id: string, updates: Partial<Omit<Seller, 'id' | 'profile_id' | 'created_at' | 'updated_at'>>)` → `Promise<Seller>`
  - `getOwnBuyer()` → `Promise<Buyer | null>` from `src/app/lib/supabase/buyers.ts`
  - `updateBuyer(id: string, updates: Partial<Omit<Buyer, 'id' | 'profile_id' | 'created_at' | 'updated_at'>>)` → `Promise<Buyer>`
  - `US_STATES` from `src/app/lib/data/usStates.ts` → `{ code: string; label: string }[]`
  - `Loader` from `src/shared/components/loader/loader` — renders a full-block spinner when `block` prop is true
  - `ProtectedRoute` from `src/app/utils/protectedRoute` — HOC wrapping the default export
  - Types: `Seller`, `Buyer`, `BusinessType`, `WorkType`, `ManagementType`, `BuyerBusinessType`, `BuyerWorkType`, `ManagementPreference` from `src/app/lib/types.ts`
  - SCSS classes from Task 1: `account-page`, `account-email`, `account-section`, `account-section__header`, `account-section__title`, `account-fields`, `account-field`, `account-field__label`, `account-field__value`, `account-field-row`, `account-chips`, `account-chip`, `account-edit-form`, `account-edit-field`, `account-edit-row`, `account-radio-group`, `account-radio-btn`, `account-radio-btn--selected`, `account-states-grid`, `account-state-btn`, `account-state-btn--selected`, `account-edit-actions`, `account-btn`, `account-btn--primary`, `account-btn--ghost`, `account-banner`, `account-banner__text`, `account-banner__link`, `account-error`
- Produces: the fully functional `/account` page

- [ ] **Step 1: Replace page.tsx with the complete implementation**

Write the following as the full content of `src/app/account/page.tsx`:
...
Expected: no errors. If TypeScript complains about the `setField` dynamic key pattern, replace the affected `onChange` with an explicit inline handler:
```tsx
...
git commit -m "feat: rewrite account page with role-aware inline edit for sellers and buyers"
```
...
```bash
cd /Users/valentingonzalez/Proyects/brokerage && npm run dev
```

- [ ] **Step 2: Verify seller flow**

  1. Log in as a seller who completed onboarding
  2. Navigate to `/account`
  3. Confirm: email is shown at the top, "Mi negocio" section appears in read mode with all fields populated
  4. Click "Editar" — confirm all fields become editable and are pre-filled with the current values
  5. Change one field (e.g. software name), click "Guardar"
  6. Confirm: mode returns to read and the updated value is shown

- [ ] **Step 3: Verify buyer flow**

  1. Log in as a buyer who completed onboarding
  2. Navigate to `/account`
  3. Confirm: email shown, "Mi Buy Box" section in read mode with all fields
  4. Click "Editar" — fields editable, pre-filled
  5. Toggle a state on/off, click "Guardar"
  6. Confirm: read mode returns with the updated state list

- [ ] **Step 4: Verify incomplete profile banner**

  1. Log in as a seller or buyer whose `sellers`/`buyers` record was deleted (or create a test user)
  2. Navigate to `/account`
  3. Confirm: banner "No tenés un perfil completo." appears with "Completar perfil →" button
  4. Click the button — confirm redirect to `/seller/onboarding` or `/buyer/onboarding`

- [ ] **Step 5: Verify cancel**

  1. Click "Editar", change several fields, click "Cancelar"
  2. Confirm: read mode returns and original values are unchanged

- [ ] **Step 6: Commit verification note**

```bash
git commit --allow-empty -m "chore: verify account edit page — all flows confirmed"
```
