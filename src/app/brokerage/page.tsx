                  ...
                  <td className="py-3">
                    {deletingId === seller.id ? (
                      <span className="flex items-center gap-2">
                        ...
                        <button className="text-xs px-2 py-1 rounded bg-red-500 text-white hover:bg-red-600 transition-colors" onClick={() => handleDelete(seller.id)} type="button">Confirm</button>
                        <button className="text-xs px-2 py-1 rounded border border-white/20 hover:border-white/50 transition-colors" onClick={() => setDeletingId(null)} type="button">Cancel</button>
                      </span>
                    ) : (
                      <span className="flex items-center gap-2">
                        <button className="text-xs px-2 py-1 rounded border border-white/20 hover:border-white/50 transition-colors" onClick={() => router.push(`/brokerage/listings/${seller.id}/edit`)} type="button">Edit</button>
                        <button className="text-xs px-2 py-1 rounded border border-red-500/30 text-red-400 hover:border-red-400 transition-colors" onClick={() => setDeletingId(seller.id)} type="button">Delete</button>
                      </span>
                    )}
                  </td>
                </tr>
